...
fun main() {
    val passwordsWithPolicy = listOf(
    ...
    var validExactPasswords = 0
    val policyValidation = "(\\d+)-(\\d+)\\s(\\w):\\s(.+)".toRegex()
    passwordsWithPolicy.forEach { passwordWithPolicy ->

        val matchResults = policyValidation.find(passwordWithPolicy)
        if (matchResults != null) {
            val groupValues = matchResults.groupValues
            val policy = Policy(groupValues[1].toInt(), groupValues[2].toInt(), groupValues[3].single(), groupValues[4])
            if (policy.isValid()) {
                validPasswords++
            }
            if (policy.isExactMatch()) {
                println(policy.toString())
                validExactPasswords++
            }
        }
    }
